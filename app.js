...
// Returns an array of {label, value} stat objects for a given player + role
function getStatsForDepthChart(player, role) {
...
const v = row[key];
return (v !== null && v !== undefined && v !== “”) ? { label, value: String(v) } : null;
}
...
return `<div class="stat-pill-row">${stats.map((s) => `<span class="stat-pill"><span class="stat-pill-label">${escapeHtml(s.label)}</span><span class="stat-pill-value">${escapeHtml(s.value)}</span></span>` ).join("")}</div>`;
}
...
document.getElementById(“rotationList”).innerHTML = state.roster.rotation.map((p) => `<div class="staff-row"> <div class="row-pos">${escapeHtml(p.primaryPos)}</div> <div class="row-name-line"> <div class="row-name" title="${escapeAttr(p.name)}">${escapeHtml(p.name)}${p.isProtected ? '<span class="protected-star">★</span>' : ''}</div> ${renderStatPills(getStatsForDepthChart(p, "starter"))} </div> </div>`).join(””);

document.getElementById(“bullpenList”).innerHTML = state.roster.bullpen.map((p) => `<div class="staff-row"> <div class="row-pos">${escapeHtml(p.primaryPos)}</div> <div class="row-name-line"> <div class="row-name" title="${escapeAttr(p.name)}">${escapeHtml(p.name)}${p.isProtected ? '<span class="protected-star">★</span>' : ''}</div> ${renderStatPills(getStatsForDepthChart(p, "bullpen"))} </div> </div>`).join(””);
...
target.innerHTML = `${visible.map((player) =>`
<div class="depth-row">
...
`).join("")} ${extra > 0 ? `<div class="pos-more">+${extra} more</div>`: ""}`;
}
...
container.innerHTML = Object.entries(state.roster).map(([groupKey, players]) => `<div class="editor-card"> <div class="editor-card-head"> <h3>${rosterMeta[groupKey]}</h3> <button class="add-row-btn" data-group="${groupKey}">Add Player</button> </div> <div class="editor-card-body" data-group-body="${groupKey}"> ${players.map((player) => renderPlayerRow(player, groupKey)).join("")} </div> </div>`).join(””);
...
list.innerHTML = txs.map((t) => `<div class="transaction-card"> <div class="transaction-main"> <div class="transaction-topline"> <div class="transaction-opponent">${escapeHtml(t.opponent || "Unknown Opponent")}</div> <div class="result-badge ${resultClass(t.result)}">${escapeHtml(t.result || "—")}</div> <div class="transaction-date">${escapeHtml(t.date || "")}</div> </div> <div class="transaction-details"> <div><strong>Acquired:</strong> ${escapeHtml(t.acquiredPlayer || "—")}</div> <div><strong>Primary Position:</strong> ${escapeHtml(t.acquiredSlot || "—")}</div> <div><strong>Removed:</strong> ${escapeHtml(t.removedPlayer || "—")}</div> <div><strong>Notes:</strong> ${escapeHtml(t.notes || "—")}</div> </div> </div> <div class="transaction-actions"> <button class="edit-btn" data-action="edit-transaction" data-id="${t.id}">Edit</button> <button class="delete-btn" data-action="delete-transaction" data-id="${t.id}">Delete</button> </div> </div>`).join(””);
...
return statsTypeView === “hitters” ? “hitter” : “pitcher”;
}

function getOurTeamRowsByType(typeLabel) {
const currentRoster = Object.values(state.roster).flat();
const rosterNames = new Set(currentRoster.map((p) => normalize(p.name)));
return statsView.filter((row) => rosterNames.has(normalize(row.name)) && row.type === typeLabel);
}

function getRealAtlRowsByType(typeLabel) {
return statsView.filter((row) => row.isRealAtl && row.type === typeLabel);
}
...
if (scope === “our_team”) {
rows = getOurTeamRowsByType(typeLabel);
} else if (scope === “real_atl”) {
rows = getRealAtlRowsByType(typeLabel);
} else {
rows = rows.filter((row) => row.type === typeLabel);
}
...
function buildTeamTotals(rows, type = getCurrentTypeLabel()) {
...
function getRealAtlLeagueRow(typeLabel) {
...
function numericMetricValue(obj, key) {
if (!obj) return null;
const val = obj[key];
...
if (pct !== null) {
percentileHtml = `<div class="percentile-wrap"> <div class="percentile-fill" style="width:${pct}%"></div> </div> <div class="percentile-label">${pct}th pct</div>`;
}
}

return `<td class="metric-cell ${betterClass}"> <span class="metric-value">${escapeHtml(String(displayValue))}</span> ${percentileHtml} </td>`;
}
...
container.innerHTML = `<div class="stats-table-wrap"> <table class="comparison-table"> <thead> <tr> <th>Team</th> ${metrics.map((m) =>`<th>${escapeHtml(m.label)}</th>`).join("")} </tr> </thead> <tbody> <tr> <td><strong>Our Team</strong></td> ${metrics.map((m) => { const sourceObj = m.source === "hitter" ? ourHitters : ourPitchers; const compareObj = m.source === "hitter" ? realHitters : realPitchers; const leagueRows = m.source === "hitter" ? hitterLeagueRows : pitcherLeagueRows; return renderMetricCell(sourceObj, m.key, leagueRows, true, compareObj, true); }).join("")} </tr> <tr> <td><strong>Real ATL</strong></td> ${metrics.map((m) => { const sourceObj = m.source === "hitter" ? realHitters : realPitchers; const compareObj = m.source === "hitter" ? ourHitters : ourPitchers; const leagueRows = m.source === "hitter" ? hitterLeagueRows : pitcherLeagueRows; return renderMetricCell(sourceObj, m.key, leagueRows, true, compareObj, true); }).join("")} </tr> <tr> <td><strong>MLB Avg</strong></td> ${metrics.map((m) => { const sourceObj = m.source === "hitter" ? leagueAvgHitters : leagueAvgPitchers; return renderMetricCell(sourceObj, m.key, m.source === "hitter" ? hitterLeagueRows : pitcherLeagueRows, false, null, false); }).join("")} </tr> </tbody> </table> </div> `;
}
...
const typeLabel = getCurrentTypeLabel();
const filteredRows = getFilteredStatsRows();

const ourHitterTotals = buildTeamTotals(getOurTeamRowsByType(“hitter”), “hitter”);
...
const displayRows = teamRow ? [teamRow, …filteredRows] : filteredRows;

tbody.innerHTML = displayRows.length
? displayRows.map((row, i) => `<tr class="${i === 0 && teamRow ? "team-row" : ""}"> ${columns.map((col) => renderStatsCell(row, col.key)).join("")} </tr>`).join(””)
...
sheet.innerHTML = `<div class="export-header"> <div class="export-header-left"> <img src="./logo.png" alt="Braves Today logo" class="export-logo" /> <div> <div class="export-title">${escapeHtml(title)}</div> <div class="export-subtitle">${escapeHtml(subtitle)}</div> </div> </div> <div class="export-tag">${escapeHtml(tagText)}</div> </div>`;
...
const tableHtml = `<table> <thead> <tr> ${columns.map((col) =>`<th>${escapeHtml(col.label)}</th>`).join("")} </tr> </thead> <tbody> ${displayRows.map((row) => `
<tr>
${columns.map((col) => {
if (col.key === “type”) {
const label = row.name === “TEAM” ? “team” : row.type;
...
list.innerHTML = txs.map((t) => `<div class="export-line-item"> <strong>${escapeHtml(t.date || "—")}</strong> • <strong>${escapeHtml(t.opponent || "—")}</strong> • <strong>${escapeHtml(t.result || "—")}</strong> • Added: ${escapeHtml(t.acquiredPlayer || "—")} • Removed: ${escapeHtml(t.removedPlayer || "—")} • Notes: ${escapeHtml(t.notes || "—")} </div>`).join(””);
